    ...
    if [[ -n "$search_term" ]]; then
        ...
        commands=$(jq -r --arg keyword "$search_term" '
            if .commands and (.commands | type == "array") then
                ...
                if length > 0 then
                    to_entries | .[] | "\(.key + 1). \(.value.name // "未命名")"
                else
                    empty
                end
            else
...
show_main_interface() {
    ...
    # 快速运行提示
    echo -e "${gl_lv}${LIGHTNING} 快速运行提示:${gl_bai}"
    echo -e "  • 输入关键词搜索命令"
    echo -e "  • 输入 '0' 退出程序"
    echo ""
...
show_main_interface() {
    ...
    # 快速运行提示
    echo -e "${GREEN}${LIGHTNING} 快速运行提示:${RESET}"
    echo -e "  • 输入数字直接执行命令 (无需确认)"
    echo -e "  • 输入关键词搜索命令"
    echo -e "  • 输入 'm' 进入管理模式"
    echo -e "  • 输入 'q' 退出程序"
    echo ""
